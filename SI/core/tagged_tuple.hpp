...
/**
 * The tagged tuple allows to use types as index for tuple. 
 *
 * e.g. 
 *
 * struct Test {};
 * tagged_tuple<Test,float> tup;
 * tup.get<Test> = 1.0f;
 *
 * use getTuple() to get a std-conform representation without the types
 *
 * shameless copy from http://stackoverflow.com/questions/13065166/c11-tagged-tuple
 */
...
namespace stdext
{
	...
	struct tt_impl<typelist<Ss...>, typelist<Ts...>> :	public std::tuple < Ts... > 
	{
		...
		tt_impl(Args &&...args) : std::tuple<Ts...>(std::forward<Args>(args)...) {}

		template<typename S>
		nth<index<S, Ss...>::value, Ts...>& get() 
		{
			return std::get<index<S, Ss...>::value>(*this);
		}

		template<std::size_t S>
		typename std::tuple_element<S, typename std::tuple<Ts...> >::type& get()
		{
